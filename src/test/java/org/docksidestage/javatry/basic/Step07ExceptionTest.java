...
public class Step07ExceptionTest extends PlainTestCase {
  ...
  /\           |     |        PK           ^^v
        ...
        // (次の週で、改めてここを深掘り、同じレイヤー内ではtry/catchはしなくてもいいかな: さすがに面倒だし)
    ...
    private void helpSurprisedYabaiCatch() {
        try {
            ...
            // done jflute ジュニアとシニアでは「読んだらわかる」の基準が違うことも不要なコメントが残ってしまう要因になるのかなと思いました。
            //  ジュニアエンジニアは自分基準で考えるのではなく、「シニアエンジニアの〇〇さんが読んだらわかるかな？」と考えると良さそう。
            // #1on1: なかなかジュニアがシニアを想定するのが難しいかも。とはいえ、その意識を持ち続けるのは良いことだと思う。 (2026/03/06)
            // そもそも、他人がどう思うかな？ってのを考える習慣を続けること自体がとても良い。(多少外してもそれも経験)
            // https://jflute.hatenadiary.jp/entry/20180625/repeatablecomment
            // (言い訳コメントも良い訳)
            // #1on1: コメントをAIが読むかもしれない話 (2026/03/06)
            // 逆に、古くて間違ったコメントがあると、AIくんが惑わされるかも。 by のにわさん
    ...
    public void test_exception_zone_differenceExceptionError() {
        // done jflute 次回1on1ここから (2026/02/20)
        // _/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/
        ...
        // _/_/_/_/_/_/_/_/_/_/
        //
        // #1on1: 例外とエラーという言葉の話 (2026/03/06)
        //
        // Error ::「投げられる時は」というキーワードが適している。
        // つまり、投げた瞬間ダメだとわかっている。エラーだとわかっている。
        // だから、最初からErrorというクラス名。
        //
        // Exception :: 「投げる瞬間」は、
        // 予期できる不具合 == エラーか？
        // 正常パスではない分岐 by のにわさん
        // yes, 正常なレアケース by jflute のこともある。
        // SoldOutExceptionは業務例外であって、システム例外ではない。かも？
        // 極論、呼び出し側の状況によって、それがシステム例外なのか業務例外なのか、変わる。
        // なので、Exceptionと言われるものは、throwしてる人はそれが「ダメ」なのかはわからない、
        // ただ単に「いま自分の業務を遂行することができないですよ」としか言わずにthrowするだけ。
        // そして、呼び出し側がそれをcatchして初めて、ダメなのかダメじゃないのか判断できる。
        // 
        // 例外は、throwする人と、catchする人、二人の登場人物がいることを意識。
        // Errorは、throwした瞬間にダメであることがわかる、からErrorというクラス。
        // 他の言語: e.g. TypeScript, Ruby, Go (システムエラー？業務エラー？)
        //
        // (平たく言った場合の)エラーハンドリングというものの、仕組みの輪郭は、
        // どの言語でも同じ。表現が違うだけで、やりたいこと同じ。
        //
        // 会話上は、例外とエラーの使い分けも、システムエラーと業務エラーの使い分けもぐちゃぐちゃになりがち。
        // それを前提に話を聞いたりした方が良い。本気で区別する必要のあるときは、厳密に使いわける。
        
        // #1on1: エラーハンドリング上手になるには？ (2026/03/06)
        // まず、こういう概念の解釈を一度インプットしておくこと。
        //
        // そもそもシステム例外と業務例外の区別。
        // 
        // 本当にエラーのものだけエラーログに
        // https://dbflute.seasar.org/ja/lastaflute/howto/architecture/exceptionhandling.html#realerroronly
    }
}
